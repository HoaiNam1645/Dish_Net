...
export async function seedNhatKyHeThong(context: SeederContext) {
  ...
  const requestIds = Array.from(context.requestByKey.values()).map((item) => item.id);
  ...
  const ycMoCuaHangChoDuyet = context.requestByKey.get(
    buildRequestKey(getUserId(context, 'user@dishnet.vn'), 'mo_cua_hang', 'cho_duyet'),
  );
  const ycCreatorChoDuyet = context.requestByKey.get(
    buildRequestKey(getUserId(context, 'creator@dishnet.vn'), 'kiem_tien_noi_dung', 'cho_duyet'),
  );
  const ycCreatorDaDuyet = context.requestByKey.get(
    buildRequestKey(getUserId(context, 'multi@dishnet.vn'), 'kiem_tien_noi_dung', 'da_duyet'),
  );
  const ycMoCuaHangTuChoi = context.requestByKey.get(
    buildRequestKey(getUserId(context, 'locked@dishnet.vn'), 'mo_cua_hang', 'da_tu_choi'),
  );
  ...
  const logs = [
    ...
    ycMoCuaHangTuChoi
      ? {
          ...
          id_doi_tuong: ycMoCuaHangTuChoi.id,
          hanh_dong: 'tu_choi',
          noi_dung: 'Thông tin cửa hàng chưa đầy đủ và số điện thoại liên hệ không hợp lệ.',
          du_lieu_cu: { trang_thai: 'cho_duyet' },
