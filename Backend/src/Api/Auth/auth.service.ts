...
export class AuthService {
  ...
  async dangKy(dto: DangKyDto) {
    if (dto.mat_khau !== dto.xac_nhan_mat_khau) {
      throw new BadRequestException('Mat khau va xac nhan mat khau khong trung khop');
    }

    const emailTonTai = await this.nguoiDungRepo.findOne({ where: { email: dto.email } });
    if (emailTonTai) {
      throw new ConflictException('Email da duoc su dung');
    }

    if (dto.so_dien_thoai) {
      const sdtTonTai = await this.nguoiDungRepo.findOne({ where: { so_dien_thoai: dto.so_dien_thoai } });
      if (sdtTonTai) {
        throw new ConflictException('So dien thoai da duoc su dung');
      }
    }

    const tenDangNhap = dto.email.split('@')[0] + '_' + Date.now().toString().slice(-4);
  ...
  async xacNhanDangKy(dto: XacNhanOtpDto) {
    const nguoiDung = await this.nguoiDungRepo.findOne({ where: { email: dto.email } });
    if (!nguoiDung) {
  ...
  async dangNhap(dto: DangNhapDto, ip?: string, userAgent?: string) {
    const nguoiDung = await this.nguoiDungRepo.findOne({
      where: [
        { email: dto.tai_khoan },
        { ten_dang_nhap: dto.tai_khoan },
      ],
    });

    if (!nguoiDung) {
      throw new UnauthorizedException('Ten dang nhap/ email hoac mat khau khong dung');
    }
    ...
    if (nguoiDung.trang_thai_tai_khoan === 'cho_xac_thuc') {
      throw new UnauthorizedException('Tai khoan chua duoc xac thuc. Vui long xac nhan OTP.');
    }
    ...
    return this.taoPhienDangNhap(nguoiDung, vaiTro, dto.luu_dang_nhap ?? false, ip, userAgent);
  }

  async dangNhapVoiVaiTro(email: string, vaiTro: string, ip?: string, userAgent?: string) {
  ...
  async quenMatKhau(dto: QuenMatKhauDto) {
    ...
    await this.maXacThucRepo.update(
      { id_nguoi_dung: nguoiDung.id, loai_xac_thuc: 'quen_mat_khau', trang_thai: 'hieu_luc' },
      { trang_thai: 'da_huy' },
    );
    ...
    await this.emailService.guiOtpQuenMatKhau(dto.email, maOtp, nguoiDung.ten_hien_thi);
  ...
  async datLaiMatKhau(dto: DatLaiMatKhauDto) {
    if (dto.mat_khau_moi !== dto.xac_nhan_mat_khau) {
      throw new BadRequestException('Mat khau moi va xac nhan mat khau khong trung khop');
    }
    ...
    const otp = await this.timOtpHieuLuc(nguoiDung.id, 'quen_mat_khau', dto.ma_otp);
  ...
  async guiLaiOtp(dto: GuiLaiOtpDto) {
    ...
    await this.maXacThucRepo.update(
      { id_nguoi_dung: nguoiDung.id, loai_xac_thuc: dto.loai_xac_thuc, trang_thai: 'hieu_luc' },
      { trang_thai: 'da_huy' },
    );
    ...
    if (dto.loai_xac_thuc === 'dang_ky') {
      await this.emailService.guiOtpDangKy(dto.email, maOtp, nguoiDung.ten_hien_thi);
  ...
  private async taoPhienDangNhap(
    ...
    const hetHanMs = ghiNho ? 30 * 24 * 60 * 60 * 1000 : 7 * 24 * 60 * 60 * 1000;
