...
export class AdminPromotionService {
  ...
  async layDanhSach(query: DanhSachKhuyenMaiQueryDto) {
    ...
    if (query.trang_thai) {
      qb.andWhere('km.trang_thai = :trangThai', { trangThai: query.trang_thai });
    }
    ...
    return {
      du_lieu: items.map((item) => ({
        ...
        so_luot_toi_da: item.so_luot_toi_da === null ? null : Number(item.so_luot_toi_da),
  ...
  async capNhat(
    ...
    await this.ghiNhatKy('cap_nhat_khuyen_mai', khuyenMai.id, actor, diaChiIp, duLieuCu, {
      ten_khuyen_mai: khuyenMai.ten_khuyen_mai,
      ma_khuyen_mai: khuyenMai.ma_khuyen_mai,
      loai_khuyen_mai: khuyenMai.loai_khuyen_mai,
      gia_tri_khuyen_mai: khuyenMai.gia_tri_khuyen_mai,
  ...
  async tamDung(
    ...
    if (this.daKetThuc(khuyenMai.thoi_gian_ket_thuc)) {
      ...
      await this.khuyenMaiRepo.save(khuyenMai);
      throw new BadRequestException('Khuyen mai da ket thuc, khong the tam dung');
    }
    ...
    await this.ghiNhatKy('tam_dung_khuyen_mai', khuyenMai.id, actor, diaChiIp, duLieuCu, {
      trang_thai: khuyenMai.trang_thai,
    });
  ...
  private async chuanHoaDuLieu(payload: TaoKhuyenMaiDto | CapNhatKhuyenMaiDto) {
    ...
    if (loaiKhuyenMai === 'phan_tram' && (giaTriKhuyenMai <= 0 || giaTriKhuyenMai > 100)) {
      throw new BadRequestException('Khuyen mai phan tram phai trong khoang 1 den 100');
    }
    ...
    if (loaiKhuyenMai === 'mien_phi_van_chuyen' && giaTriKhuyenMai !== 0) {
      throw new BadRequestException('Mien phi van chuyen phai co gia tri bang 0');
    }
    ...
    if (Number.isNaN(thoiGianBatDau.getTime()) || Number.isNaN(thoiGianKetThuc.getTime())) {
      throw new BadRequestException('Thoi gian khuyen mai khong hop le');
    }

    if (thoiGianKetThuc <= thoiGianBatDau) {
      throw new BadRequestException('Thoi gian ket thuc phai lon hon thoi gian bat dau');
    }
