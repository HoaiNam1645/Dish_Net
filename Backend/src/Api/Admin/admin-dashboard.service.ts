...
export class AdminDashboardService {
  ...
  private async layThongKeTongQuanTrongKhoang(range: DateRange) {
    const [tongNguoiDung, tongCuaHang, tongDonHang, doanhThuRaw] = await Promise.all([
      this.nguoiDungRepo
  ...
  private async layBieuDoDoanhThuTheoThang(range: DateRange) {
    ...
    const start = new Date(range.tu_ngay.getFullYear(), range.tu_ngay.getMonth(), 1);
    const end = new Date(range.den_ngay.getFullYear(), range.den_ngay.getMonth(), 1);
    const data: Array<{
      thang: string;
      nhan: string;
      doanh_thu: number;
  ...
  private async layTopMonAn(range: DateRange) {
    const rows = await this.donHangChiTietRepo
      ...
      .addSelect('MAX(COALESCE(ma.hinh_anh_dai_dien, ct.hinh_anh_snapshot))', 'hinh_anh')
  ...
  private xayDungKhoangThoiGian(query: ThongKeHeThongQueryDto): DateRange {
    const boLoc = (query.bo_loc_thoi_gian || 'today') as DateRange['bo_loc_thoi_gian'];
    const now = new Date();
    ...
    switch (boLoc) {
      case 'today': {
        ...
        return { bo_loc_thoi_gian: 'today', tu_ngay: tuNgay, den_ngay: denNgay };
      }
      case '7days': {
        const tuNgay = new Date(now);
        tuNgay.setDate(now.getDate() - 6);
        tuNgay.setHours(0, 0, 0, 0);
        return { bo_loc_thoi_gian: '7days', tu_ngay: tuNgay, den_ngay: denNgay };
      }
      case '30days': {
        const tuNgay = new Date(now);
        tuNgay.setDate(now.getDate() - 29);
        tuNgay.setHours(0, 0, 0, 0);
        return { bo_loc_thoi_gian: '30days', tu_ngay: tuNgay, den_ngay: denNgay };
      }
      case 'custom': {
        ...
        if (Number.isNaN(tuNgay.getTime()) || Number.isNaN(denNgayCustom.getTime())) {
