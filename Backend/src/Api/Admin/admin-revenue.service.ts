...
export class AdminRevenueService {
  ...
  async layTongQuan() {
    const [tongQuan, bieuDoTheoNgay, bieuDoTheoNguon, topCuaHang, topNhaSangTao] =
      await Promise.all([
  ...
  private async layThongKeTongQuan() {
    ...
    const [tongDoanhThuRaw, doanhThuHomNayRaw, tongSoDonHang] = await Promise.all([
  ...
  private async layBieuDoDoanhThuTheoNgay() {
    ...
    const rows = await this.donHangRepo
      ...
      .andWhere('COALESCE(dh.thoi_gian_hoan_tat, dh.thoi_gian_dat) BETWEEN :tuNgay AND :denNgay', {
        tuNgay: from,
        denNgay: to,
      })
      .groupBy("DATE_FORMAT(COALESCE(dh.thoi_gian_hoan_tat, dh.thoi_gian_dat), '%Y-%m-%d')")
      .orderBy("DATE_FORMAT(COALESCE(dh.thoi_gian_hoan_tat, dh.thoi_gian_dat), '%Y-%m-%d')", 'ASC')
      .getRawMany<{ ngay: string; doanh_thu: string }>();

    const byDate = new Map(rows.map((item) => [item.ngay, Number(item.doanh_thu)]));
    const data: Array<{
      ngay: string;
      nhan: string;
      doanh_thu: number;
  ...
  private async layBieuDoDoanhThuTheoNguon() {
    ...
    const tong = totals.video_review + totals.tim_kiem_mon_an + totals.khuyen_mai;
    return [
      {
        key: 'video_review',
        nhan: 'Video review',
  ...
  private async tinhTongDoanhThu(tuNgay?: Date, denNgay?: Date) {
    ...
    if (tuNgay && denNgay) {
      qb.andWhere('COALESCE(dh.thoi_gian_hoan_tat, dh.thoi_gian_dat) BETWEEN :tuNgay AND :denNgay', {
        tuNgay,
        denNgay,
      });
    }
